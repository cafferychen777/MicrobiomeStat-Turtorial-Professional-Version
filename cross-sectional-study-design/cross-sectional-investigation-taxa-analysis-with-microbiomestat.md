...
Before visualizing abundance distribution using heatmaps, we can also generate exploratory plots like boxplots to compare taxa abundance between groups with `generate_taxa_boxplot_single`.
...
<figure><img src="../.gitbook/assets/Screenshot 2023-08-05 at 19.38.33.png" alt=""><figcaption><p>This boxplot, generated by MicrobiomeStat's <code>generate_taxa_boxplot_single</code> function, visually showcases the distribution of microbial genus abundance between LGG and placebo groups at time point 1. The white lines represent the median abundance, while the violin shapes showcase the abundance distribution. Genera like Akkermansia and Parabacteroides exhibit lower abundance among LGG samples compared to placebo. This plot swiftly highlights patterns of differential abundance between groups, priming us for subsequent in-depth analysis.</p></figcaption></figure>

This function subsets the data, filters low abundance taxa, and transforms abundance values. Then it generates a faceted boxplot for each taxonomic level, with samples grouped and colored by `group.var`.

The boxplots provide an initial overview of how abundance distributions for top taxa differ between groups. They help identify interesting taxa for further investigation using statistical tests and heatmaps.
